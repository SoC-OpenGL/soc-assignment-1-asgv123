...
//1 face, 64 mini-sq/face, 2 triangles/mini-sq, 3 vertices/triangle
const int num_vertices = 2*3*64;

//Eight vertices in homogenous coordinates
glm::vec4 positions[81];

void fill_pos()
{
  for(int i = 0; i < 81; i++)
  {
    positions[i] = glm::vec4(0.1*(i%9), 0.1*(i/9), 0.0, 1.0);
  }
}
...
{
  v_colors[tri_idx] = colors[col]; v_positions[tri_idx] = positions[a]; tri_idx++;
  v_colors[tri_idx] = colors[col]; v_positions[tri_idx] = positions[b]; tri_idx++;
  v_colors[tri_idx] = colors[col]; v_positions[tri_idx] = positions[c]; tri_idx++;
  v_colors[tri_idx] = colors[col]; v_positions[tri_idx] = positions[b]; tri_idx++;
  v_colors[tri_idx] = colors[col]; v_positions[tri_idx] = positions[c]; tri_idx++;
  v_colors[tri_idx] = colors[col]; v_positions[tri_idx] = positions[d]; tri_idx++;
 }

// generate 128 triangles:
void colorcube(void)
{
  int sq_cnt = 0, clr;
  while(sq_cnt < 64)
  {
    int bot_row = sq_cnt/8;
    int left_col = sq_cnt%8;
    int low_left = bot_row*9 + left_col;
    if((bot_row+left_col)%2==0) clr=0;
    else clr=1;
    quad(low_left, low_left+1, low_left+9, low_left+10, clr);
    sq_cnt++;
  }
}
//-----------------------------------------------------------------
